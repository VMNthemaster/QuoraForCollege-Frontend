  ...
  useEffect(() => {
    if(searchQuestion){
      const searchQuestionRequestToBackend = async () => {
        const url = `https://quora-for-college-backend.onrender.com/api/questions/${school}/searchQuestion`
      ...
      searchQuestionRequestToBackend().then(data => {
        if(data.success){
          setAllQuestions(data.updatedQuestions)
        }
      })
    }
  ...
  return (
    <div className="min-h-[91vh] bg-gray-100 ">
      <div className="h-[5vh] w-[94vw] md:w-[60vw] border-[0.5px] md:border-2 border-red-500 rounded-md flex items-center mx-auto ">
        ...
        <div className="w-[100%] pr-[1vw] py-[0.5vh]">
          <input
            className="w-[100%] outline-none text-sm md:text-xl bg-gray-100 placeholder-gray-700 placeholder:font-serif"
            type="text"
            placeholder="Write keywords for better chances of solution"
            name='searchQuestion'
      ...
      {!isLoading && (
        <div className="flex flex-col flex-wrap justify-start items-center w-[94vw] md:w-[70vw] md:px-[1.5vw] min-h-[86vh] py-[1vw] mx-auto gap-y-[1.5vh] ">
            ...
            allQuestions.map((question) => {
              return (
                <div
                  ...
                  className="flex justify-between w-[100%] border-[0.5px] md:border-[2px]  rounded-md cursor-pointer bg-gradient-to-r from-red-400 to-pink-400 text-white hover:bg-gradient-to-l clickEffect"
                  key={question._id}
                >
          ...
          <div
            ...
            className="fixed bottom-[7vh] right-[10vw] md:right-[20vw] z-10 p-[1rem] rounded-full bg-gray-200 cursor-pointer text-lg md:text-2xl border-[0.5px] md:border-2 border-black clickEffect"
          >
